...
def generate_insert_script_from_json(json_file):
    ...
    for item in data:
        ...
        image_url = item.get("imageURL", "").replace("'", "''")[:500]  # จำกัดความยาว 500 ตัวอักษร
        ...
        categories = [cat.strip() for cat in category.split(",") if cat.strip()]  # ลบช่องว่างและหมวดหมู่ว่าง
        for seq, cat in enumerate(categories, start=1):
            fccode = categories.index(cat) + 1  # รหัสหมวดหมู่เริ่มต้นที่ 1
            foodcategorydetail_insert = f"INSERT INTO foodcategorydetail (fdcode, seq, fccode) VALUES ({fdcode},{seq},{fccode});"
        ...
        for seq, (nutrient, details) in enumerate(nutrients.items(), start=1):
            ...
            if amount != 0:  # ใส่เฉพาะค่าที่ไม่เป็นศูนย์
                ntcode = details["ntcode"]
                unit = details["unit"]
                nutrientdetail_insert = f"INSERT INTO nutrientdetail (fdcode, seq, ntcode, amount, unit) VALUES ({fdcode},{seq},{ntcode},{amount},'{unit}');"
